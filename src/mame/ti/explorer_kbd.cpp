...
That microcontroller's ROM has not been dumped, so this device is a
high-level stand-in.

Known protocol bytes:
- 0x00 sent by the SIB is a keyboard initialization/reset code. The
  keyboard is expected to reply with 0x70 to acknowledge it.
- Key transmission: a key-down sends its scancode with the high bit
  set (scancode | 0x80); a key-up sends the bare scancode.
...
static INPUT_PORTS_START( explorer_kbd )
	...
	// Keyboard keys that produce mouse button characters, they are not wired to
	// the SIB's own mouse port.
	...
	PORT_BIT(0x0004, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD =") PORT_CODE(KEYCODE_EQUALS_PAD) // 062
	PORT_BIT(0x0008, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD +") PORT_CODE(KEYCODE_PLUS_PAD) // 063
	PORT_BIT(0x0010, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD SPACE") // 064
	PORT_BIT(0x0020, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD TAB") PORT_CODE(KEYCODE_TAB_PAD) // 065
	...
	PORT_BIT(0x0100, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 7") PORT_CODE(KEYCODE_7_PAD) // 110
	PORT_BIT(0x0200, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 8") PORT_CODE(KEYCODE_8_PAD) // 111
	PORT_BIT(0x0400, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 9") PORT_CODE(KEYCODE_9_PAD) // 112
	PORT_BIT(0x0800, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD -") PORT_CODE(KEYCODE_MINUS_PAD) // 113
	...
	PORT_START("ROW6")
	PORT_BIT(0x0001, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 4") PORT_CODE(KEYCODE_4_PAD) // 140
	PORT_BIT(0x0002, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 5") PORT_CODE(KEYCODE_5_PAD) // 141
	PORT_BIT(0x0004, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 6") PORT_CODE(KEYCODE_6_PAD) // 142
	PORT_BIT(0x0008, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD ,") PORT_CODE(KEYCODE_COMMA_PAD) // 143
	...
	PORT_BIT(0x0040, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 1") PORT_CODE(KEYCODE_1_PAD) // 166
	PORT_BIT(0x0080, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 2") PORT_CODE(KEYCODE_2_PAD) // 167
	PORT_BIT(0x0100, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 3") PORT_CODE(KEYCODE_3_PAD) // 170
	...
	PORT_BIT(0x2000, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD 0") PORT_CODE(KEYCODE_0_PAD) // 175
	PORT_BIT(0x4000, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD .") PORT_CODE(KEYCODE_DEL_PAD) // 176
	PORT_BIT(0x8000, IP_ACTIVE_HIGH, IPT_KEYBOARD) PORT_NAME("KEYPAD ENTER") PORT_CODE(KEYCODE_ENTER_PAD) // 177
INPUT_PORTS_END
