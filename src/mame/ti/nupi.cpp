...
TODO:
A nupi interface can have multiple formatters attached. Each formatter
can have 2 devices: hard disk(s) or tape(s).
The formatters are the scsi devices, not the disks/tapes themselves.
The current setup should behave like a system with 6 formatters each
having only a device 0.

The documentation referencing the disk formatter mentions 'adaptec'. It
looks there may be an adaptec scsi interface involved to interface
the winchester disks.
