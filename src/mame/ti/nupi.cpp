...
void nupi_device::device_start()
{
	...
	// formatter trace.
	for (char const *tag : { "scsibus:0:harddisk", "scsibus:1:harddisk", "scsibus:2:harddisk" })
	{
		if (auto *hd = dynamic_cast<nscsi_harddisk_device *>(subdevice(tag)))
			hd->set_seek_timing(4000, 26000, 43000, 3600, 1);
	}
...
void nupi_device::ram_window_w(offs_t offset, u32 data, u32 mem_mask)
{
	...
	if (offset * 4 == 4 && (mem_mask & 0xff000000))
	{
		...
		if (hits < 2000)
		{
			...
			logerror("CMDLOG cmd_addr=%08X word0=%08X cmd=%02X unit=%02X (fmt=%d dev=%d) dst_bit=%d formatter_bit=%d buffer_ptr=%08X word_count=%08X block_addr=%08X event_addr=%08X\n",
			...
			printf("CMDLOG cmd_addr=%08X word0=%08X cmd=%02X unit=%02X (fmt=%d dev=%d) dst_bit=%d formatter_bit=%d buffer_ptr=%08X word_count=%08X block_addr=%08X event_addr=%08X\n",
				cmd_addr, word0, (word0 >> 24) & 0x3f, word0 & 0xff, (word0 >> 3) & 0x7, word0 & 0x1,
				BIT(word0, 31), BIT(word0, 30),
				buffer_ptr, word_count, block_addr, event_addr);
...
void nupi_device::device_add_mconfig(machine_config &config)
{
	...
	NSCSI_CONNECTOR(config, "scsibus:0", nupi_scsi_devices, "harddisk", false); // unit 00
	NSCSI_CONNECTOR(config, "scsibus:1", nupi_scsi_devices, "harddisk", false); // unit 08
	NSCSI_CONNECTOR(config, "scsibus:2", nupi_scsi_devices, "harddisk", false); // unit 10
	NSCSI_CONNECTOR(config, "scsibus:3", nupi_scsi_devices, nullptr, false);    // reserved for tape drive??
