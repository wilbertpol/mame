...
TIMER_CALLBACK_MEMBER(electron_state::electron_tape_timer_handler)
{
	if ( m_ula.cassette_motor_mode )
	{
		...
		// Cassette input handling copied from bbc driver
		if ((tap_val >= 0.0 && m_ula.last_tap_val < 0.0) || (tap_val < 0.0 && m_ula.last_tap_val >= 0.0))
		{
			...
			m_ula.len3 = m_ula.len2;
			m_ula.len2 = m_ula.len1;
			m_ula.len1 = m_ula.len0;
			m_ula.len0 = m_ula.tap_val_length;

			m_ula.tap_val_length = 0;

			if ((m_ula.len0 + m_ula.len1) >= (18+18-5))
			{
			...
			if (((m_ula.len0 + m_ula.len1 + m_ula.len2 + m_ula.len3) <= 41) && (m_ula.len3 != 0))
			{
...
void electron_state::machine_start()
{
	...
	save_item(STRUCT_MEMBER(m_ula, len0));
	save_item(STRUCT_MEMBER(m_ula, len1));
	save_item(STRUCT_MEMBER(m_ula, len2));
	save_item(STRUCT_MEMBER(m_ula, len3));
